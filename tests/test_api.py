...
def test_exposed_objects(modulelevel_internal_objects: list[griffe.Object | griffe.Alias]) -> None:
    """All public objects in the internal API are exposed under `terminator_dbus`."""
    not_exposed = [
        obj.path
        for obj in modulelevel_internal_objects
        if obj.name not in terminator_dbus.__all__ or not hasattr(terminator_dbus, obj.name)
    ]
...
def test_single_locations(public_api: griffe.Module) -> None:
    ...
    multiple_locations = {}
    for obj_name in terminator_dbus.__all__:
...
def test_inventory_matches_api(
    ...
    public_api_paths.add("terminator_dbus")
    for item in inventory.values():
        if (
            item.domain == "py"
            and "(" not in item.name
            and (item.name == "terminator_dbus" or item.name.startswith("terminator_dbus."))
        ):
